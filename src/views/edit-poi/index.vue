<template>
  <van-nav-bar left-text="返回" left-arrow @click-left="goBack" title="编辑商户"> </van-nav-bar>

  <van-field v-model="currentItem.name" label="商户名称" placeholder="请输入商户名称" :rules="rules.name" />

  <van-field v-model="currentItem.phone" type="phone" label="电话号码" placeholder="请输入电话号码" :rules="rules.phone" />

  <van-field v-model="ereaText" is-link readonly label="地区选择" placeholder="点击选择省市区" :columns-placeholder="['请选择', '请选择', '请选择']" @click="showArea = true" />
  ...
  <van-popup v-model:show="showMap" w-full>
    <input-position h-60 @confirm-location="onConfirmLocation" @cancel-location="showMap = false"></input-position>
  </van-popup>

  <van-field v-model="currentItem.address" type="address" label="详细地址" placeholder="请输入详细地址" :rules="rules.address" />

  <van-field type="status" label="营业状态" placeholder="请输入营业状态" :rules="rules.status" label-align="top">
    <template #input>
  ...
  >
    <template #icon>
      <van-icon name="add-o" block p-r-1 p-t-0.5 @click.stop="onAdd" v-if="isAdd" />
    </template>
  </van-cell>
...
<!-- 
  <BusinessTime bg-white :business-hours="times"></BusinessTime>

  <button class="hover: px-1.5 py-2 flex items-center bg-white border-none w-full text-0.6em"  v-if="times.length < 2"
  @click="
      () => {
        editTimeShow = true
      }"
  >
    <span class="iconify" data-icon="bx:bx-plus" p-r-0.5></span>
    <div>添加营业时间</div>
  </button> -->
...
<!-- 
  ...
  <van-popup v-model:show="editTimeShow" round position="bottom" @edit-time-cancel="editTimeShow = false" @edit-time-confirm="onEditTimeConfirm" h-70>
    <edit-time @editTimeConfirm="onEditTimeConfirm" @editTimeCancel="editTimeShow = false"></edit-time>
  </van-popup>

  <div flex>
    <van-button w-full type="primary" block @click="onUpdate"> 提交 </van-button>
...
import { updatePOI } from '../../api/poi'

import { bindUpdatePOIItem } from '../../store'
import { bTime, times , newTime} from '../../store'
import BusinessTime from '../../components/BusinessTime.vue'
...
bindUpdatePOIItem({ yid })

// const choosedTimePeriod = computed(() => currentItem.value.businessTime.filter((item) => item.checked))
...
const onAddSegment = () => {
  ...
  rt.weekday = rt.weekday.filter(item=>!choosed.includes(item))
